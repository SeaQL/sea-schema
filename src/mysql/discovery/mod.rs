...
impl SchemaDiscovery {
    ...
    pub async fn discover(mut self) -> Schema {
        ...
        let tables = future::join_all(
            tables.into_iter().map(|t| (&self, t)).map(Self::discover_table_static)
        ).await;
    ...
    pub async fn discover_tables(&mut self) -> Vec<TableInfo> {
        let rows = self.executor.fetch_all(self.query.query_tables(self.schema.clone())).await;
    ...
    pub async fn discover_table(&self, info: TableInfo) -> TableDef {
        ...
        let columns = self.discover_columns(self.schema.clone(), table.clone()).await;
        let indexes = self.discover_indexes(self.schema.clone(), table.clone()).await;
        let foreign_keys = self.discover_foreign_keys(self.schema.clone(), table.clone()).await;
    ...
    pub async fn discover_columns(&self, schema: Rc<dyn Iden>, table: Rc<dyn Iden>) -> Vec<ColumnInfo> {
        let rows = self.executor.fetch_all(self.query.query_columns(schema.clone(), table.clone())).await;
    ...
    pub async fn discover_indexes(&self, schema: Rc<dyn Iden>, table: Rc<dyn Iden>) -> Vec<IndexInfo> {
        let rows = self.executor.fetch_all(self.query.query_indexes(schema.clone(), table.clone())).await;
        ...
        let indexes = parse_index_query_results(Box::new(results.into_iter())).map(|index| {
            debug_print!("{:?}", index);
            index
        }).collect::<Vec<_>>();
        debug_print!();
    ...
    pub async fn discover_foreign_keys(&self, schema: Rc<dyn Iden>, table: Rc<dyn Iden>) -> Vec<ForeignKeyInfo> {
        let rows = self.executor.fetch_all(self.query.query_constraints(schema.clone(), table.clone())).await;
        ...
        let foreign_keys = parse_constraint_query_results(Box::new(results.into_iter())).map(|index| {
            debug_print!("{:?}", index);
            index
        }).collect::<Vec<_>>();
        debug_print!();
